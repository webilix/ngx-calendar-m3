...
export class NgxCalendarDateComponent implements OnInit, OnChanges {
    ...
    @Input({ required: false }) timezone?: string;
    @Output() onChange: EventEmitter<INgxCalendarDate> = new EventEmitter<INgxCalendarDate>();

    public values!: { today: string; selected: string; minDate: string; maxDate: string; timezone: string };
    ...
    initValues(): void {
        let timezone: string = this.timezone || this.config?.timezone || 'Asia/Tehran';
        timezone = this.jalali.timezones().includes(timezone) ? timezone : 'Asia/Tehran';
        ...
        this.values = {
            today: this.jalali.toString(new Date(), { format: 'Y-M-D', timezone }),
            selected: this.value ? this.jalali.toString(this.value, { format: 'Y-M-D', timezone }) : '',
            minDate: this.minDate ? this.jalali.toString(this.minDate, { format: 'Y-M-D', timezone }) : '0000-00-00',
            maxDate: this.maxDate ? this.jalali.toString(this.maxDate, { format: 'Y-M-D', timezone }) : '9999-99-99',
            timezone,
        };

        const month: string = this.jalali.toString(this.value || new Date(), { format: 'Y-M', timezone: timezone });
    ...
    setDate(value: string): void {
        ...
        const timezone: string = this.values.timezone;
        const gregorian: string = this.jalali.gregorian(value).date;
        const date: Date = this.jalali.periodDay(1, new Date(gregorian + 'T00:00:00.000Z'), timezone).from;
        const jalali: string = this.jalali.toString(date, { format: 'Y-M-D', timezone });

        this.onChange.next({ timezone, date, jalali });
    }
}
